...
if __name__ == '__main__':
    ...
    # plt.plot(z,eig(z))
    # print(eig(0))
    fs = np.linspace(0,10e9,201,dtype=np.complex128)
    mc = np.array(list(map(lambda f:R1.mode_condition(f), fs)))
    # print(mc)
    # jac = np.array(list(map(lambda k:R1.matrix_condition_derivative(k), ks)))
    # # print(R1.matrix_condition_derivative(100))
    fig, ax = plt.subplots(1,1)
    ax.plot(fs,mc[:,0], label='mode condition', color='tab:blue')
    # ax2 = ax.twinx()
    ...
    # ax.legend()
    # ax2.legend()
    ax.plot(fs,mc[:,1], label='mode condition', color='tab:orange')
    # ax2 = ax.twinx()
