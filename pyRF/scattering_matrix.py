...
class CapacitorMatrix:
    ...
    def scattering_matrix(k, position, direction_array, characteristic_impedance, phase_velocity, capacitance):
        ...
        if len(direction_array) == 1:
            scattering_matrix = np.array([[(1 - characteristic_impedance * zc_inv) / (1 + characteristic_impedance * zc_inv) * np.exp(-4j * np.pi * direction_array[0] * k * position)]])
        else:
    ...
    def guess_phase(k, characteristic_impedance, phase_velocity, capacitance, **_):
        print('phase',2 * np.arctan(2 * np.pi * k * phase_velocity * capacitance * characteristic_impedance))
